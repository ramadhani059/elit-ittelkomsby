...
<div class="container-xxl flex-grow-1 container-p-y">
  ...
  <div class="row">
    <div class="col-md-12">
      <div class="card mb-4">
        <!-- Account -->
        <form action="{{ route('information-admin.store') }}" method="POST" enctype="multipart/form-data" id="AksesJurnalForm">
        @csrf
            <div class="card-body">
            <div class="d-flex align-items-start align-items-sm-center gap-4">
                <img
                    src="{{ asset('assets/img/informasi/Informasi.png') }}"
                    ...
                    <div class="input-group">
                        ...
                        <select class="form-select status @error('status') is-invalid @enderror" id="status" name="status">
                            ...
                            @foreach($status as $statusinfo)
                                <option value="{{ $statusinfo }}" {{ old('status') == $statusinfo ? 'selected' : '' }}>{{ $statusinfo }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div id="defaultFormControlHelp" class="form-text text-danger">
